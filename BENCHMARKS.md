...
| **PostgreSQL** | 17.x, Docker container (linux/arm64) |
| **Shared buffers** | 256 MB |
| **Work mem** | 8 MB |
| **Effective cache size** | 512 MB |
| **WAL buffers** | 16 MB |
| **Max WAL size** | 1 GB |
| **Max connections** | 200 |
| **Benchmark concurrency** | 50 parallel HTTP requests |
| **Benchmark DB pool** | 50 connections (seeding only) |
...
*Results below are from a previous single-threaded DB-direct run and will be
updated after the first parallel HTTP benchmark run.*
...
50,000 parallel create calls (80% new distinct_ids, 20% existing).
...
### Phase 3 — Merge (POST /merge, batched)
...
Builds realistic chain depth distribution by merging targets into each other.
Not timed as part of the benchmark — this is fixture preparation.

### Phase 4 — Read (POST /resolve)

1,000,000 parallel random lookups of non-primary distinct_ids.
...
and the entire working set fits in shared_buffers. Reads bypass the worker channel
entirely and go direct to the DB pool.

### Phase 5 — Delete distinct_id (POST /delete_distinct_id)

10,000 parallel delete calls for pre-seeded distinct_ids.

*Results pending first parallel HTTP run.*

### Phase 6 — Delete person (POST /delete_person)

10,000 parallel delete calls for pre-seeded persons.
...
Entire working set fits in the 256 MB shared_buffers with room to spare.
...
- Cold reads (cache miss on NVMe): ~0.1 ms per random 8K page read, so worst case
  adds ~2 ms per lookup (6 potential page faults x 0.1 ms + seek time)
